...
/*
    ...
    (무작정 다 조회를 해온은게 아니라 각 테이블간 연결고리로써의 데이터를 매칭해서 조회해야됨!!)
...
-- >> 오라클 전용 구문
-- FROM절에 조회하고자 하는 테이블들 싹다 나열 (, 구분자로)
...
-- DEPT_CODE가 NULL인 사원 조회 X , DEPT_ID D3, D4, D7 조회 X => 둘다 있어야 조회 가능!!
...
-- 해결방법2) JOIN USING 구문 사용하는 방법 (** 두 컬럼명이 일치할 때만 사용 가능!!!)
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
...
-- 2. DEPARTMENT과 LOCATION 을 참고해서 전체 부서의 부서코드, 부서명, 지역코드, 지역명 조회
...
-- >> ANSI 구문
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY * 12
FROM EMPLOYEE -- EMPLOYEE에 있는건 다 나오는 거임
...
-- 2. 사번, 사원명, 부서명, 직급명, 지역명, 국가명, 해당 급여등급에서 받을 수 있는 최대 금액(다 조인)
SELECT * FROM EMPLOYEE;
SELECT * FROM DEPARTMENT;
SELECT * FROM LOCATION;
