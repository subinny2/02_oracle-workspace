...
SELECT EMPNO, ENAME,
--CASE WHEN SAL >= 1000 THEN RPAD(SUBSTR(SAL,1,2),4,0) 
--END
TRUNC(SAL,-2)
FROM EMP
ORDER BY SAL DESC;
...
SELECT ENAME, SUBSTR(HIREDATE,1,2)||'년' AS "입사년도" ,
              SUBSTR(HIREDATE,4,2)||'월' AS "입사월",
...
--18. EMP테이블에서 입사일로부터 38년 이상 근무한 직원의 정보 조회 --456개월이상
SELECT *
FROM EMP
