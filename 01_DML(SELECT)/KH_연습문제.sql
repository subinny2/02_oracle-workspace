...
SELECT SUBSTR(EMP_NO,1,2) AS "하이유의 생년", SUBSTR(EMP_NO,3,2) AS "하이유의 생월", SUBSTR(EMP_NO,5,2) AS "하이유의 생일"
FROM EMPLOYEE
WHERE EMP_NAME = '하이유';
...
SELECT EMP_NAME, FLOOR(MONTHS_BETWEEN(SYSDATE,HIRE_DATE)) AS "근속개월수"
FROM EMPLOYEE
...
SELECT EMP_ID, EMP_NAME, SUBSTR(EMP_NO,1,6) AS "앞자리", SUBSTR(EMP_NO,8,14)AS"뒷자리", TO_NUMBER(SUBSTR(EMP_NO,1,6))+ TO_NUMBER(SUBSTR(EMP_NO,8,14)) AS "앞자리 뒷자리 합"
FROM EMPLOYEE
WHERE EMP_ID = '201';
