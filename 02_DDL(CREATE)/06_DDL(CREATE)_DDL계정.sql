...
CREATE TABLE MEM(
    ...
    PHONE VARCHAR2(13),
    EMAIL VARCHAR2(50),
    GRADE_ID NUMBER REFERENCES MEM_GRADE --(GRADE_CODE) -- 컬럼레벨방식 : 생략하면 (부모테이블의)프라이머리키로 자동매칭!!
    -- FOREIGN KEY(GRADE_CODE) REFERENCES MEM_GRADE
);
...
INSERT INTO MEM
VALUES(3, 'user03', 'pass03', '이강인', null, null, null, 40);
-- ORA-02291: integrity constraint (DDL.SYS_C007119) violated - parent key not found ( FK제약조건 )
...
WHERE GRADE_CODE = '10'; -- ****************
-- ORA-02292: integrity constraint (DDL.SYS_C007119) violated - child record found
-- > 자식테이블(MEM)에 10 이라는 값을 사용하기 있기 때문에 삭제가 안됨!!!
...
/*
    ...
    * 삭제옵션 : 부모테이블의 데이터 삭제시 그 데이터를 사용하고 있는 자식 테이블의 값을
                어떻게 처리할건지
    
    - ON DELETE RESTRICTED(기본값) : 삭제제한옵션으로, 자식 데이터로 쓰이는 부모데이터는 삭제 아예 안되게끔!!!
    - ON DELETE SET NULL : 부모 데이터 삭제시 해당 데이터를 쓰고있는 자식데이터의 값을 NULL로 변경한다.
...
CREATE TABLE MEM(
    ...
    PHONE VARCHAR2(13),
    EMAIL VARCHAR2(50),
    GRADE_ID NUMBER REFERENCES MEM_GRADE(GRADE_CODE) ON DELETE SET NULL -- 컬럼레벨방식 : 생략하면 (부모테이블의)프라이머리키로 자동매칭!!
);
...
CREATE TABLE MEM(
    ...
    PHONE VARCHAR2(13),
    EMAIL VARCHAR2(50),
    GRADE_ID NUMBER REFERENCES MEM_GRADE(GRADE_CODE) ON DELETE CASCADE -- 컬럼레벨방식 : 생략하면 (부모테이블의)프라이머리키로 자동매칭!!
);
...
-- 선택되지 않은 컬럼에는 기본적으로는 NULL이 들어감!
-- 단, 해당 컬럼에 DEFAULT값이 있을경우 NULL이 아닌 DEFAULT값이 들어감!!!
...
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
    ...
    WHERE 1 = 0; -- 무조건 FALSE 인조건 : 구조만을 복사하고자 할 때 쓰이는 구문 (데이터 값은 필요 없을 때)
...
-- EMPLOYEE 테이블에 SAL_LEVEL에 외래키 제약조건 추가 (SAL_GRADE 테이블 참조)
